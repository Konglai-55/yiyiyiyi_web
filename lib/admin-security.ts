...
export async function privateId(text:string) {
  const secret=(await config()).ADMIN_SESSION_SECRET;
...
export async function jsonBody(request:Request) {try{const data=JSON.parse(new TextDecoder().decode(await limitedBody(request)));if(!data||typeof data!=='object'||Array.isArray(data))throw new HttpError(400,'提交格式不正确');return data;}catch(e){if(e instanceof HttpError)throw e;throw new HttpError(400,'提交格式不正确');}}
export async function sameOrigin(request:Request) {
  const origin=(await config()).APP_ORIGIN || new URL(request.url).origin;
  if(request.headers.get('origin')!==origin) throw new HttpError(403,'请从本站后台操作');
}
export async function clientAddress(request:Request) {
  const env=await config();
  if(env.DB) return request.headers.get('cf-connecting-ip')||'unknown';
  // Only trust a header overwritten by the deployment's reverse proxy.
  return env.TRUST_PROXY==='1' ? (request.headers.get('x-real-ip')||'unknown') : 'shared';
}
export async function rateLimit(key:string,limit:number,seconds:number) {
  const now=Math.floor(Date.now()/1000);
  const result=await (await db()).prepare('INSERT INTO rate_limits (key,count,expires) VALUES (?,1,?) ON CONFLICT(key) DO UPDATE SET count=CASE WHEN expires <= ? THEN 1 ELSE count+1 END, expires=CASE WHEN expires <= ? THEN excluded.expires ELSE expires END RETURNING count').bind(key,now+seconds,now,now).first<{count:number}>();
...
export async function requireAdmin(request:Request) {
  ...
  const session=await (await db()).prepare('SELECT expires FROM admin_sessions WHERE token=? AND expires>?').bind(await digest(token),Math.floor(Date.now()/1000)).first();
  if(!session) throw new HttpError(401,'登录已过期，请重新登录');
}
export async function sessionCookie(request:Request,token:string,maxAge=28800) {return `yixiao_admin=${token}; Path=/; HttpOnly; SameSite=Strict; Max-Age=${maxAge}${new URL((await config()).APP_ORIGIN||request.url).protocol==='https:'?'; Secure':''}`;}
export async function verifyPassword(password:string) {
  const setting=await (await db()).prepare('SELECT value FROM settings WHERE key=?').bind('admin_password_hash').first<{value:string}>();
  const encoded=setting?.value || (await config()).ADMIN_PASSWORD_HASH;
