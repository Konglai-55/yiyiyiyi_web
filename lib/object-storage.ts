...
export async function uploadImage(request:Request) {
  ...
  const c=config();if(!c.S3_ACCESS_KEY_ID||!c.S3_SECRET_ACCESS_KEY)throw new HttpError(503,'图片存储未配置');
  ...
  const uploaded=await aws.fetch(`${c.S3_ENDPOINT}/${c.S3_BUCKET}/${key}`,{method:'PUT',headers:{'Content-Type':type,'Cache-Control':'public, max-age=31536000, immutable'},body:bytes});
