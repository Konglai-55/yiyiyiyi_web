...
export default async function CaseDetail({ params }: Props) {
  ...
  return <NewsShell section="cases">
    ...
    <section className="case-story"><div className="c-shell case-story-grid"><aside><h2>项目概况</h2><dl><dt>项目区域</dt><dd>{project.location}</dd><dt>应用场景</dt><dd>{project.scene}</dd><dt>资料来源</dt><dd>{project.source}</dd></dl><a className="case-consult" href="/contact">咨询同类项目</a></aside><div className="case-story-copy"><p className="case-intro">{project.intro}</p><figure className="case-platform case-platform-inline"><a href={project.image} target="_blank" rel="noopener noreferrer" aria-label={`打开${project.title}完整截图`}><img src={project.image} alt={`${project.title}平台监测界面`} width="1900" height="980" /></a><figcaption><span>项目平台实景</span><a href={project.image} target="_blank" rel="noopener noreferrer">查看完整截图 ↗</a></figcaption></figure>{detailSections.length > 0 && <div className="case-detail-sections">{detailSections.map((section, index) => <div key={section.title}><section><h2>{section.title}</h2><p>{section.body}</p></section>{gallery[index] && <figure className="case-inline-gallery"><a href={gallery[index].src} target="_blank" rel="noopener noreferrer" aria-label={`查看${gallery[index].alt}`}><img src={gallery[index].src} alt={gallery[index].alt} loading="lazy" data-lightbox /></a><figcaption>{gallery[index].alt}</figcaption></figure>}</div>)}</div>}{gallery.length > detailSections.length && <section className="case-gallery case-gallery-inline" aria-label={`${project.title}项目现场图片`}><div className="case-gallery-heading"><p>PROJECT EVIDENCE</p><h2>项目现场与应用实景</h2><span>项目资料实拍 · 现场图片仅用于展示项目建设与运行环境</span></div><div className="case-gallery-grid case-gallery-count-2">{gallery.slice(detailSections.length).map(item => <figure key={item.src}><a href={item.src} target="_blank" rel="noopener noreferrer" aria-label={`查看${item.alt}`}><img src={item.src} alt={item.alt} loading="lazy" data-lightbox /></a><figcaption>{item.alt}</figcaption></figure>)}</div></section>}<h2>项目内容与管理重点</h2><ul>{project.work.map(item=><li key={item}>{item}</li>)}</ul><h2>平台运行展示</h2><p>{platformNotes[slug]}</p><p className="case-evidence-note">图中设备数量、在线率及告警统计为截图时点的信息，不代表当前实时运行数据。</p><div className="case-links"><a href="/products">了解平台与设备体系 ↗</a><a href="/products/smoke">了解 AI 大数据烟感 ↗</a></div></div></div></section>
