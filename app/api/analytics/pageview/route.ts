...
import { jsonBody,privateId,rateLimit,response,routeError,sameOrigin,HttpError } from '@/lib/admin-security';
export async function POST(request:Request) {
  try {
    ...
    await rateLimit(`view:${await privateId(request.headers.get('cf-connecting-ip')||data.session)}`,120,60);
    ...
    await db().batch([
      db().prepare('INSERT OR IGNORE INTO page_views (id,day,path,visitor,device,referrer,created) VALUES (?,?,?,?,?,?,?)').bind(data.id,day,path,await privateId(data.session),/mobile|android|iphone|ipad/i.test(ua)?'移动端':'桌面端',referrer,Math.floor(Date.now()/1000)),
      db().prepare('DELETE FROM page_views WHERE day<?').bind(new Date(Date.now()-90*86400000).toISOString().slice(0,10)),
      db().prepare('DELETE FROM rate_limits WHERE expires<?').bind(Math.floor(Date.now()/1000)-86400),
    ]);
