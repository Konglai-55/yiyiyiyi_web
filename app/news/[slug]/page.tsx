...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return article ? { title: `${article.title}｜新闻资讯｜壹壹壹壹`, description: article.summary, alternates: { canonical: `/news/${article.slug}` }, openGraph: { type: 'article', url: `/news/${article.slug}`, title: article.title, description: article.summary, publishedTime: article.publishedAt, images: article.cover ? [article.cover] : undefined } } : { title: '文章未找到' };
}
export default async function NewsDetail({ params }: Props) {
  ...
  const jsonLd = { '@context': 'https://schema.org', '@type': 'Article', headline: article.title, description: article.summary, datePublished: article.publishedAt, dateModified: article.updatedAt || article.publishedAt, author: { '@type': 'Organization', name: '壹壹壹壹（北京）安全技术有限公司' }, publisher: { '@id': 'https://yiyiyiyi.xn--fiqs8s/#organization' }, mainEntityOfPage: `https://yiyiyiyi.xn--fiqs8s/news/${article.slug}`, image: article.cover || undefined };
  return <NewsShell>
